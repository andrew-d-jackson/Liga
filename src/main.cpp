...
// Set the global so the code gen can use this.
 ...
 print->create_mapping(env, std::make_shared<FloatType>());
    ...
    print->create_vector_mapping(env, std::make_shared<VectorType>(std::make_shared<CharType>()));
  ...
  auto builder = llvm::IRBuilder<>(main_entry);
    ...
    auto test_parse = parse("[print [at 1 (\"ab\" \"cd\")]]");
  ...
  for (const auto &i : test_parse) {
   ...
   /// builder.CreateRet(builder.getTrue());
    std::cout <<
    global_scope.values.size();
  global_scope.create_return(env, builder, GenericValue{ std::make_shared<BooleanType>(),
                                              builder.getTrue()});
