...
public:
    ...
    llvm::Function* destructor(Enviroment &env, llvm::IRBuilder<> &builder, GenericValue val) const {
        static std::map<GTPtr, llvm::Function*> fn_map;
        if (fn_map.find(val.type) == fn_map.end()) {
            auto fn_ty = llvm::FunctionType::get(llvm::Type::getVoidTy(llvm::getGlobalContext()), {val.type->llvm_type()}, false);
            auto fn = llvm::Function::Create(fn_ty, llvm::Function::ExternalLinkage, "destruct", env.module);
            auto entry = llvm::BasicBlock::Create(llvm::getGlobalContext(), "entry", fn);
            auto build = llvm::IRBuilder<>(entry);
            for_each_element(env, build, val.type->create(fn->arg_begin()), [&](GenericValue v) {
                v.type->destroy(env, build, v);
            });
